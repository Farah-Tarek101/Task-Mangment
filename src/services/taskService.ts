...
export async function createTask(
  ...
  await assertProjectExists(projectId, userId);
...
export async function listTasksByProject(
  ...
  if (sortResult.isPrioritySort) {
    ...
    const data =
      paginated.map(task => formatTask(task));
  ...
  const sort = {
    [sortResult.sortBy]: sortResult.sortOrder
  };
  const [tasks,total] = await Promise.all([
...
export async function updateTask(
  ...
  if(body.project_id !== undefined){
    ...
    throw new AppError(
      'project_id cannot be changed after task creation',
      400
    );
